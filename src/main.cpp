...
unsigned int nTargetSpacing = 2 * 60; // 1 minute blocks
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // 24 hours
unsigned int nStakeMaxAge = -1;           //unlimited
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 40; //120 blocks for coins 
...
const string strMessageMagic = "QuitDough Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	 ...
	 int64_t nSubsidy = 100 * COIN;
        ...
        {
                nSubsidy = 3 * 1000 * 1000 * COIN;		
        }
		else if(pindexBest->nHeight >= 2 && pindexBest->nHeight < 150) 
        {
				nSubsidy =  1 * COIN;
		}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "QuitDough", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "http://www.coindesk.com/markets-weekly-price-crosses-300-following-coinbase-news/";
        CTransaction txNew;
        txNew.nTime = 1422303275 ;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1422303275;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1672306;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x0b44cd1f5004116d13819a7eb4fd1e206f00bf06f84dbfb540ffd9fba448ecd1"));
        //block.print();
